...
def has_stable_eps_growth_cagr(ticker):
    try:
        ...
        selected_years = [
            year for year in eps_years if current_year - 5 <= year <= current_year - 1
        ]

        # Filter eps_series to selected years only
        filtered_eps = eps_series[
            [col for col in eps_series.index if col.year in selected_years]
        ]
        ...
        if eps_start <= 0 or eps_end <= 0:
            ...
            stable_growth = all(
                ...
                for earlier, later in zip(eps_list, eps_list[1:])
            )
...
def ensure_cache_1y_for_tickers(tickers, cache_file="yf_cache_multi.csv"):
    ...
    if not cache.empty:
        ...
        start_date = today - pd.tseries.offsets.BDay(
            252
        )  # fallback: 252 business days ago
...
def get_operating_income_yoy(ticker_obj):
    try:
        ...
        if all(
            later >= earlier for earlier, later in zip(income_values, income_values[1:])
        ):
            return True
...
def process_ticker_quantitatives():
    while not q.empty():
        ...
        try:
            ...
            pbr = info.get(
                "priceToBook", None
            )  # 초점: 자산가치, 저pbr종목은 저평가된 자산 가치주로 간주. 장기 수익률 설명력 높음 < 1.5 (=being traded at 1.5 times its book value (asset-liab))
            per = info.get(
                "trailingPE", None
            )  # 초점: 수익성, over/undervalue? 저per 종목 선별, 10-20전후(혹은 산업평균)로 낮고 높음 구분. 주가가 그 기업의 이익에 비해 과대/과소평가되어 있다는 의미
            ...
            div_growth = has_stable_dividend_growth_cagr(
                yf_ticker
            ...
            quantitative_buffett_score = buffett_score(
                debtToEquity,
                currentRatio,
                pbr,
                per,
            ...
            intrinsic_value_score = score_intrinsic_value(
                *intrinsic_value_range,
                currentPrice,
                fcf_yield,
                tenyr_treasury_yield,
                fcf_cagr,
            )
            ...
            roe_print += (
                f"({round(industry_roe*100)})" if industry_roe is not None else ""
            )

            roa_print = f"{round(roa*100,1)}%" if roa is not None else "N/A"
            roa_print += (
                f"({round(industry_roa*100)})" if industry_roe is not None else ""
            )
            ...
            operating_income_yoy = (
                f"{operating_income_yoy:.0%}"
                if isinstance(operating_income_yoy, float)
                else operating_income_yoy
            )
            operating_income_qoq = (
                f"{operating_income_qoq:.0%}"
                if isinstance(operating_income_qoq, float)
                else operating_income_qoq
            )
            ...
            result = {
                ...
                "추정DCF": (
                    f"${intrinsic_value_range[0]:,.0f} - ${intrinsic_value_range[1]:,.0f}"
                    if intrinsic_value_range and intrinsic_value_range != (None, None)
                    else "N/A"
                ),
                "부채비율": (
                ...
                "배당성장률": (
                    div_growth
                    if isinstance(div_growth, bool)
                    else (f"{div_growth:.2%}" if div_growth is not None else "N/A")
                ),
                "영업이익률(Y/Q)": str(
                ...
                + "/"
                + str(
                    operating_income_qoq if operating_income_qoq is not None else "N/A"
                ),
                "모멘텀": momentum_score,
                # '거래량': vol,
...
def generate_moat_summary(df: pd.DataFrame, moat_limit: int) -> pd.DataFrame:
    ...
    for ticker in top_tickers:
        try:
            ...
            moat_data.append({"기업명": ticker, "경쟁 우위 분석": moat_text})
...
if isinstance(cache.columns, pd.MultiIndex):
    ...
    if "Close" in cache.columns:
        ...
        close_df.columns = (
            [tickers[0]] if "tickers" in locals() and tickers else ["Close"]
        )
    else:
...
**아래 제공된 뉴스 요약 및 검색할 수 있는 정보를 바탕으로** 다음 작업을 수행해 주세요.  
